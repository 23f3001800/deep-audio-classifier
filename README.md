...
# 🎵 Music Genre Classifier — AST V2

Fine-tuned [Audio Spectrogram Transformer (AST)](https://huggingface.co/MIT/ast-finetuned-audioset-10-10-0.4593) for 10-class music genre classification on noisy stem mashups.

Trained as part of the **Messy Mashup** project — a genre classification challenge where test audio consists of stems mixed from different songs with added environmental noise (ESC-50).
...
- **Val split:** Last 10 songs per genre held out (100 songs total, deterministic)
...
Environmental noise (ESC-50 dataset) was injected during training in 3 stages:
...
**2-phase fine-tuning**
- Phase 1 (5 epochs): Backbone frozen, only 10-class head trains at lr=3e-4
- Phase 2 (15 epochs): Full model with Layer-wise LR Decay (LLRD, decay=0.85)
...
- Optimised for noisy mashup conditions. Clean single-instrument audio may behave differently from training distribution.
...
Vikas25S — Jan 2026 Deep Learning Project
